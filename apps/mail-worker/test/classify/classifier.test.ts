...
describe('classifier', () => {
  ...
  describe('classifyMail', () => {
    ...
    describe('PDF サイズ予算', () => {
      ...
      // タイムアウトを明示するのは、このケースが 36MiB の本文を Postgres へ往復
      // させる**重量級**だから（単独実行で約 1.4 秒）。既定の 5 秒だと
      // `fileParallelism` 有効時に他 worker と CPU/IO を取り合って超えることがある
      // （2026-09-07 に 3/3 で再現）。並列化をやめる理由にはならないので、この
      // 1 件だけ余裕を持たせる。
      it('本文が巨大でリクエスト全体が 32MiB を超えるなら AI を呼ばずスキップする', { timeout: 30_000 }, async () => {
