 ...
 *
 * Member-facing counterpart of `/api/admin/mail/attachments/:id`. Streams a
 * stored mail attachment back to the browser for any logged-in member
 * (`member` / `vice_admin` / `admin` — role is not checked). This route is a
 * deliberate copy of the admin route rather than a shared helper: extracting
 * a common module would turn any future admin-route change into an implicit
 * change of this route too, and the member-mail-search requirements
 * explicitly forbid touching the admin route (Non-goals §5). The two files
 * are kept in lockstep by `attachment-route-parity.test.ts`, which asserts
 * identical response headers for identical inputs — that test is the drift
 * guard, not a shared module.
 *
 * The security policy itself is unchanged from the admin route and must stay
 * that way: mail attachments are UNTRUSTED input fetched from IMAP on behalf
 * of a sender we do not control. A hostile sender can attach `text/html` or
 * `image/svg+xml`, and rendering either one inline would be stored XSS on
 * this origin. Policy (fail-closed allowlist):
 ...
 *     Inline is required for the iOS home-screen PWA: its in-app browser
 *     cannot hand `Content-Disposition: attachment` to a download manager
 *     and instead dies on a blank page (see
 *     `feedback_ios_pwa_attachment_disposition`), so an authenticated route
 *     like this one uses the same allowlist approach as the admin route
 *     rather than the public token route's "always attachment" policy.
 *   - Everything else — active content (html / svg / xml / js …), unknown or
 *     malformed types — is rewritten to `application/octet-stream` and
 *     forced to `Content-Disposition: attachment`. Sender-controlled input
 *     cannot be made safe by enumerating known-bad types, so anything
 *     outside the allowlist fails closed to a download.
 ...
 *
 * Unlike the admin route, authorization here is `session.user.id` presence
 * only — no role check. Any logged-in member may fetch any attachment (the
