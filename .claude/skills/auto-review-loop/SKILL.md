...
argument-hint: "[PR番号(任意)] [--max-rounds N(default 3)] [--auto-ship]"
---
...
- `tokens`: stderr 末尾の `tokens used\n{N}` をパースして集計（取得できなければ null）
...
### 失敗（max-rounds 到達 or ping-pong）

1. サマリーを表示:
   ```
   ## /auto-review-loop 結果: ❌ 未収束（理由: {max-rounds 到達 | ping-pong}）
   - 残った指摘:
...
```
- {日付} /auto-review-loop PR #{番号}: {ラウンド数}R, verdict={最終verdict}, tokens={合計}, result={pass|stuck|max-reached}
```
...
- **コスト**: 1 ラウンドあたり 20k〜50k トークン消費する想定（差分サイズ次第）。max-rounds 3 で最大 150k 程度。
