...
- **同居・相互参照・非重複:** `docs/features/<slug>/` に requirements.md（本スキル）と design-spec.md（design-screen）が同居。**画面レイアウトを言葉で再記述しない**＝design-spec があれば読んで参照で済ませ、ロジック/遷移/データ/API/DB に集中する。
...
コードベース調査のうち read-heavy な探索（関連ファイル特定・既存パターン収集）は Agent tool の `Explore` サブエージェント（**`model: haiku` を明示**。スキーマ調査など精度が要るものは `model: sonnet`）に委譲して要約だけ受け取ってよい（正典: docs/dev/model-delegation.md）。技術設計の判断自体は main が行う。
...
- **機能名**: 定義した機能の名前
- **主要な設計判断**: ヒアリングで決定した重要な設計上の判断とその理由
- **作成したIssue**: 親Issue番号・URL、子Issue番号一覧
- **実装計画の要約**: 実装手順書のタスク数、主要な実装ステップ、依存関係の概要
- **要件定義書・実装手順書のパス**: `docs/features/<機能名>/requirements.md`, `docs/features/<機能名>/implementation-plan.md`

記録のカテゴリは `feature-definition` とし、機能名をタグとして付与する。
