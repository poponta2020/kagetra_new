...
- 「続けて `/implement <機能名>` を自動呼び出しします」と伝える

報告後、Skillツールで `implement` スキルを自動呼び出しする。引数には機能名を渡す。
これにより `/implement` → `/prepare-pr` → `/auto-review-loop` まで自動で繋がる。
