...
description: senseki-stats PR-2（ナビ再構成）SHIPPED。タブ改称＋4セクションシェル＋ルート scaffold。PR#221 merge・本番deploy済
metadata: 
  node_type: memory
  type: project
...
senseki-stats タスク4（#212・PR-2 ナビ）**SHIPPED**。**PR#221 merge `59f572c`（feat commit `fb9fd83`）・本番 deploy 済（2026-07-01）**。worktree/branch はクリーンアップ済。
...
**残:** 本番実機目視のみ。UI は最小 scaffold（PR-3〜5 で中身実装）。親=[[project_senseki_stats_tab]]、PR-1=[[impl_senseki_stats_pr1_derived_bracket]]、PR-3=[[impl_senseki_stats_pr3_ranking]]。
