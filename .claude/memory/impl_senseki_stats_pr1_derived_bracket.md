...
**本番反映 完了（2026-07-01）:**
- **migration 0037 は auto-deploy が自動適用済み**（PR #220 merge `7661fae` の Deploy job＝`APPLY: 0037_derived_bracket`,
  `applied=1, skipped=37`, DEPLOY_RESULT=SUCCESS）。※「dump/apply」は不要だった＝単純な additive 列なので
  通常の db:migrate 運用で足りる（series 層のような Drizzle 非管理スキーマとは別）。
- **backfill は本番ホストで直接実行して完了**。手順＝`ssh -i ~/.ssh/id_ed25519_oracle ubuntu@new.hokudaicarta.com`
  → `sudo -u kagetra bash -lc 'cd /opt/kagetra && DATABASE_URL=<.env.production から抽出> corepack pnpm --filter
  @kagetra/web exec tsx scripts/backfill-derived-bracket.ts [--dry-run]'`（ホスト上で local DB 接続＝ネットワーク転送なし・
  .env.local footgun なし・migrate と同じ環境）。**dry-run→本適用→独立 psql 読み戻し→冪等 dry-run(rows to change=0)** で検証。
- **結果**: 8,689 級 / 367,675 参加中 **derivable=301,075**（champions=7,805・入賞≤8=62,447・null=66,600）。
  分布 1:7805/2:7810/4:15619/8:31213/16:61056/32:98125/64:66536/128:12748/256:163。アプリ再起動不要
  （PR-1 は保存値未使用＝getPlayerRecord は live 導出継続・挙動不変。保存値は PR-3/PR-5 で使う）。

**次 PR＝PR-2 ナビ（#212, タスク4・PR-1 と独立）。**
