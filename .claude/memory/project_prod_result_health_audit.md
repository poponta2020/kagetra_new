...
2026-07-03 本番DB(kagetra)を **read-only** で全数点検（Tier1=DB内部のみ、原本突合なし）。成果物3点＋CSV2点を `docs/data-quality/` に格納（REPORT_本番結果データ点検 / シリーズ別年次データ有無マトリクス / class-health-metrics / suspicious-tournaments）。抽出SQL・分析スクリプトは `c:/tmp/dq/`（git外・このマシンのみ）。
