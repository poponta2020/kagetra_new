...
- 残作業: ①**HTML表パーサは W1 で実装・ship済**（[[impl_result_html_parser]], PR#167 merge `8c3ed1e`、実4510ページ回帰=例外0/相手解決99.9%）。残るパーサ作業=**W2 Excel署名検出拡張**(positional「N回戦」、既存75%通過の回帰固定必須、`round-cell.ts` 再利用)＋W3少数異形。②6414.xls 修復＋pdf24/PDF結果の扱い決定 ③**全コーパス**(static Excel375＋HTML4510＋new585)をパース→開催単位 manifest→[[reference_tool_output_fabrication]] 警戒で read-back。**2010–2026 全期間の結果調達は完了**。
