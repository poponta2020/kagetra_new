...
> **「白波 × 紺」** へ移行した。2026-09 の round 4 で、本番の実画面を見て地と枠線の彩度だけを
> 0.375 倍へ落とした（地は「水色」から「水色鼠」へ。明度ラダーは不変）。経緯と全トークンの導出は
...
- 「北溟」は『荘子』逍遥遊の「北冥有魚」、北の果ての大海を指す。これを **白波・海面・深み** の 3 層として読み、面の 3 役にそのまま割り当てる — 白波 = `surface`（カード・シート）純白、海面 = `canvas`（ページの地）水色鼠、深み = `brand`（ブランド・主要操作・肯定）紺。百人一首 76 番の結句「沖つ白波」は題材の内側にある像である。
...
| 原則 | 実装 |
|---|---|
| **White-crest surfaces** | カードは純白 `#FFFFFF`、地は水色鼠 `#DFE8ED`。**面どうしの明度差を保つこと** — surface ↔ canvas は ΔL 0.07 が下限（canvas ↔ surface-alt は 0.035）。**地の青みが強いと感じても明度を上げて消そうとしないこと** — 白へ寄せると surface との ΔL が下限を割り「のぺっとしている」が再発する。淡くするなら彩度だけを下げる。`bg-white` / `text-white` の例外 6 箇所（LINE ボタンの文字・トグルつまみ・文書プレビュー）は surface と同値になったが `bg-surface` へ置換しない — surface がどう変わっても白であるべき箇所で、意味が違う |
...
- **水色鼠 (canvas)** `#DFE8ED` / **surface** `#FFFFFF` / **recessed** `#D1DBE2`
...
| `Card` | `#FFFFFF` 背景 + `1px #CAD2D9` 枠 + 段1の 2 層シャドウ |
...
--kg-canvas:       #DFE8ED;  /* 水色鼠（海面） */
--kg-surface:      #FFFFFF;  /* 純白（白波） */
--kg-surface-alt:  #D1DBE2;

--kg-border-soft:  #CAD2D9;
--kg-border:       #B6BFC7;
--kg-border-strong:#9099A2;
...
| AI 要項取り込み (PDF/Word → イベント作成) | upload ゾーンは 12px 破線 `#B6BFC7`（border）。抽出結果は「下書き」として作成フォームにプリフィル |
...
**最終更新** : 白波 × 紺 round 4 — 地と枠線の彩度を 0.375 倍へ（2026-09。hokumei-palette）。
