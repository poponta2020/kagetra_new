...
- `b-step2-gradenull.html` — `users.grade` が NULL の会員（requirements §3.2.1・AC-5b）。既存の警告2種と同じ規約に載せ、差分は3点のみ: 警告先出し面に3件目／級セルを「未設定」（--kg-accent-fg・sans 10px。**列幅 34px は据え置き**）／集計行に「級未設定 N」（項目が増えるので折り返し可）。かなは保持されるので、かな行では警告文だけを朱にしてかな本体は通常色のまま。**空欄のままでも作成できる**ので CTA は無効化しない
...
- **モック内の個人情報・連絡先はすべてダミー値**（2026-07-27 差し替え）。会の連絡先 `karuta-club@example.com`・電話 `090-0000-0000`・責任者「山田 太郎」・主催者宛先 `aomori-karuta@example.jp` はいずれも実在しない値で、`example.com` / `example.jp` は RFC2606 の予約ドメイン。**実値をここへ戻さないこと**（docs は tracked。会の Yahoo アドレスは IMAP 資格情報のアカウント本体でもある）。所属会名・振込名義人だけは仕様上の定数なので実値のまま
...
| S1「下書き作成済 7/27 21:04・山田」 | 作成履歴テーブル（最新行） |
...
- [ ] 警告は3種すべて同じ規約（先出し面＋該当行の ⚠）に載る: ふりがな未登録／出場回数 incomplete／**参加級 未登録**。参加級 未登録の行は級セルが「未設定」（--kg-accent-fg）で列幅 34px のまま、集計行に「級未設定 N」が出て A〜E級の人数には混入せず、CTA は無効化されない（b-step2-gradenull.html）
...
- 2026-07-27: Round 1（A/B比較）→ Round 2（B案の状態網羅）で locked
- 2026-07-27: locked 後のレビューで見つかった 2 点を補完（design_source・chosen_direction・採用案は不変）:
  - **状態網羅の漏れ**: requirements §3.2.1 と「デザインへの宿題 #3」が警告 3 種を挙げているのに、モックは 2 種しか描いていなかった → `b-step2-gradenull.html` を追加（承認済みの `b-step2.html` は変更していない）
  - **PII の差し替え**: 会の電話・E-Mail・責任者実名・主催者宛先が実値のまま入っており、design-spec 自身の「架空データ」記述と食い違っていた → 全モックをダミー値へ（上記「共通の意匠判断」参照）
  - 実測（全12枚・375px）: 横スクロール 0 / トークン解決 OK。縦スクロールが出るのは `a-normal`（A案・不採用）と `b-step2` 系（15名リスト）のみで意図どおり
