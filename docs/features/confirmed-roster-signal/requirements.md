...
今回追加する2経路は**名簿の中身を持たない**ので、出場者は従来どおり出欠ベースのまま。

→ **メール連動／手動フラグだけのグループは、ホームの大会ステータスピルは「名簿確定」に
なる（ピルはこの4材料で判定する。[home-tournament-timeline](../home-tournament-timeline/requirements.md)
§3.2.2）が、名前チップは出欠「出る」の会員のまま。これは意図した挙動。**
名前のデータが無いものから出場者を組むことはできないため。将来これをバグと誤認しないよう
ここに明記し、出場者判定の不変を AC-14 で固定する。
...
- 2026-09-21: §3.2.5 を更新。メール連動／手動フラグだけのグループは、ホームのピルが
  「希望」ではなく「名簿確定」になる（名前チップは出欠ベースのまま・AC-14 は不変）
  （理由: home-tournament-timeline でホームのピルを大会の進行状況の4値へ変えたため）
