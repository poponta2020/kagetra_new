...
3. 参加者（`event_attendances.attend = true`）が **0 名**。「出欠行が無い」ではなく「`attend = true` の行が無い」— 全員が「不参加」と回答した大会も参加者 0 名として掲載する
...
| AC-22 | 出欠回答が全員「不参加」（`attend=false` のみ）の大会は、参加者 0 名として `/events-no-entrants` に表示される | auto-test |
...
- **並びを開催日の昇順にした理由**: 母集団は開催日前だけなので「次に来る大会が上」が自然で、`/events` 本体（開催日昇順）と同じ向きになる。`/events-archive` の降順は過去だけを扱うページゆえの向きで、ここでは合わない。締切日の降順（最近流れた順）も候補だったが、開催日前という短い窓に入る件数は少なく、「もうすぐ開催なのに誰も出ない」を先に見せるほうが手を打てる余地がある。
