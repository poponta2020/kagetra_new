...
- `roster-file-adoption` — 名簿をパースせず原本ファイルのまま採用できる導線（メール添付→entry_group×種別）。会員向けビューア新設・申込管理ボードの hasConfirmedRoster をファイル採用へ拡張（主要領域: packages/shared schema + apps/web admin/mail-inbox・events・roster-files）
