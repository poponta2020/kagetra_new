...
- `external-entrants-api` — match-tracker 抽選の優先枠デフォルト選択用に、大会出場予定者（人×出場日・確定/希望・ゲスト印）を返す APIキー認証の読み取り専用 API。ウィンドウ判定・名寄せ・UI は match-tracker 側（主要領域: apps/web api/external・lib/upcoming-entrants・middleware）
