...
- `hokumei-palette` — アプリ名「北溟」に合わせた配色の刷新（藤 × 墨 → 白波 × 紺）。北の大海を白波・海面・深みの 3 層として読み、surface 純白・canvas 水色・brand 紺に割り当てる。カード↔地の明度差を 0.044 → 0.075 へ広げてメリハリを付ける。朱・山吹と影の構造は据え置き。LINE Flex と統計の級トーンのリテラルも追随（主要領域: apps/web globals.css + lib/line-flex-mail-body・open-chat/flex・stats/grade-tones + docs/design）
