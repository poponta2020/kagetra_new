...
- `line-chat-commands` — 大会別 LINE グループで Bot をメンションして「申し込みました」「振り込みました」と送ると、申込グループ内の全開催日の申込ステータス・支払ステータスが進む（申込＝管理者のみ／支払＝管理者・副管理者。成功時は既存の完了通知に任せて Bot は返信しない）（主要領域: apps/web lib/line-webhook-handler + lib/line-chat-command・line-chat-authz・line-chat-command-reply + lib/events/apply-entries-applied）
