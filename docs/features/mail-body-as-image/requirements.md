...
- **本文行の内容は件名だけ**。受信日時・差出人・本文の抜粋は**載せない**。
  ...
  - 長い件名はカードが縦に伸びないよう 3 行で打ち切る（`maxLines: 3`。全文は altText と全文ページに残る）。
    加えて**カード見出しの文字列そのものを 200 字で切り詰める**（codex r1 blocker）。`maxLines` は表示行数を
    絞るだけで送信 JSON のサイズは減らさず、件名は外部メール由来で長さ無制限のため、極端に長い件名では
    Flex メッセージが LINE の 30KB 上限を超えて push が 400 で失敗し、**本文カードも添付カードも届かなくなる**。
