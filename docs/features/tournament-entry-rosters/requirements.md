...
> **2026-09-07 追記**: 案内承認画面（flow①）の系列選択は [mail-ai-extract-refinements §3.2.9〜3.2.11](../mail-ai-extract-refinements/requirements.md) で改修された（通称欄との相互連動・検索の照合対象に `short_name` 追加・回次の漢数字対応）。
>
> - **上書きされた箇所**: §3.1 と AC-1 / AC-2 の「**正規化完全一致が1件のときだけ初期選択**」に、「**名寄せ候補（完全一致＋部分一致）が1件だけのとき初期選択**」が**足された**（同書 AC-45〜AC-47 が新しい正）。完全一致が単独なら他に部分一致があっても従来どおり採用し、それ以外で候補が0件・複数件なら未選択のままという原則は変わらない。
> - **維持されている安全策**: 「名寄せは100%自動にしない」「既存系列は ID で確定し検索語を再名寄せしない」「新規作成は明示確認が必須」「種別不一致は拒否」（同書 AC-61 で回帰固定）。自動解決（`scoreSeries` / `rankSeriesCandidates` のスコアリング）は変更されていない。
