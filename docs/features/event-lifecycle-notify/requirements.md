...
3. Bot が紐付け済み参加者グループへ「申し込みが完了しました！」＋抽選日の行を push。
...
2. 締切 **3 日前 0:00**（事前）と**当日 0:00** に、未申込なら参加者グループへ催促が飛ぶ。
   - 3 日前: 「申込締切はM/D(曜)（あと3日）です。まだ申し込みが行われていません。」
   - 当日: 「⚠️申込は今日までです！⚠️」
3. 既に申込済なら何も飛ばない。締切を過ぎても再通知しない（毎朝催促はしない）。

**シナリオ C: 事前支払い（要望③・事前払い）**
1. 支払いタイプが「事前払い」（会が締切までに振込）の大会。
2. 管理者が支払状態を「支払済」にすると「参加費の振り込みが完了しました。」を push（初回のみ）。
...
2. 会レベルの「支払済」フラグは持たない。代わりに**当日持参リマインド**を送る。
   - 大会日 3 日前 0:00:「参加費は現地払いです。当日忘れないようにしてください。」
   - 大会日当日 0:00:「大会当日です！参加費を忘れないようにしてください。」
...
**2026-08-22 全面改訂。** 全種別から**大会名と金額を外した**（宛先は1グループ＝1大会なので
大会名は冗長。金額を知る必要があるのは会計だけで、その連絡は名簿確定後の
振込連絡が担う → [line-bot-message-revamp](../line-bot-message-revamp/requirements.md) §3.3）。

| 種別 | トリガー | 文面 | 1 回限り保証 |
|---|---|---|---|
| 申込完了 (`entry_applied`) | 未申込→申込済 の初回遷移（server action） | `申し込みが完了しました！`<br>（空行）<br>`抽選日は7/20(月)です。` — `lottery_date` が NULL なら `抽選日は未定です` | `event_lifecycle_notifications` の UNIQUE |
| 申込締切・事前 (`entry_deadline_advance`) | `entry_deadline = 今日+3` かつ 未申込（日次 0:00） | `申込締切は7/20(月)（あと3日）です。まだ申し込みが行われていません。` | 同上 |
| 申込締切・当日 (`entry_deadline_day`) | `entry_deadline = 今日` かつ 未申込（日次 0:00） | `⚠️申込は今日までです！⚠️` | 同上 |
| 事前支払完了 (`payment_paid`) | 事前払い かつ 未払→支払済 の初回遷移（server action） | `参加費の振り込みが完了しました。` | 同上 |
| 事前支払締切・事前 (`payment_deadline_advance`) | 事前払い かつ `payment_deadline = 今日+3` かつ 未払（日次 0:00） | `支払い締切は7/25(金)（あと3日）です。まだ振込が行われていません。` | 同上 |
| 事前支払締切・当日 (`payment_deadline_day`) | 事前払い かつ `payment_deadline = 今日` かつ 未払（日次 0:00） | `⚠️振込締切は今日までです！⚠️` | 同上 |
| 現地払い・事前 (`onsite_payment_advance`) | 現地払い かつ `event_date = 今日+3`（日次 0:00） | `参加費は現地払いです。当日忘れないようにしてください。` | 同上 |
| 現地払い・当日 (`onsite_payment_day`) | 現地払い かつ `event_date = 今日`（日次 0:00） | `大会当日です！参加費を忘れないようにしてください。` | 同上 |

- 日付は `formatEventDate`（`M/D(曜)`・ゼロ埋めなし）で整形する。
  従来の `formatMMDD`（曜日なし）から**曜日つきへ変更**した
- `⚠️` は絵文字（U+26A0 U+FE0F）。テキスト記号の `⚠` ではない
- **複数日グループ用の日別ラベル（`8/1(土)C級・8/8(土)D級` の列挙）は全種別で廃止する。**
  大会名を名乗らなくなったため、束ねた結果の文面は単一日と同一になる。
  ただし **(グループ, 種別, 基準日) による束ね処理そのものは残す** —
  3日グループで同じ締切なら通知は1通でなければならない（3通に増やしてはならない）
- 抽選日はグループ単位で同一という運用前提に立ち、日別に出し分けない
...
- **金額は文面に載せない**（2026-08-22 改訂）。`fee_jpy` / 級別単価 / 振込総額 / 級未設定の注記は
  すべて通知から外した。画面表示（大会詳細の参加費）と、名簿確定後の振込連絡には引き続き使う。
...
- 2026-08-22: 全8種別の文面を改訂し、大会名と金額を全廃。日付を曜日つきへ変更。複数日の日別ラベルを廃止（束ね処理は維持）（理由: 宛先が1グループ＝1大会で大会名が冗長、金額は会計向けの振込連絡へ集約）
