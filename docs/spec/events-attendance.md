...
セクションは上から **ヘッダー（パンくず・グループ名・開催日の並び・共通締切）→ 申込フロー帯 → 日程 → 進行管理 → 共通項目 → LINE配信 →（名簿確定フェーズのみ）振込連絡 → 名簿 → オープンチャット → 関連メール**。振込連絡は「確定名簿あり ∧ 事前払い ∧ 未振込」かつ LINE 紐付けありのときだけ現れる（文面・送信の仕様は [spec/notifications.md](notifications.md)）。ヘッダーとフロー帯は1つのラッパーで sticky にし（日ページと同じ型なので行き来しても目が迷わない）、運営操作は既定=閉の `<details>` に畳む。グループ名は `deriveEntryGroupName`（保存しない）で、導出不能なら代表イベントのタイトルへフォールバックする。
...
1. 管理者が `/admin/entries/[groupId]`（申込グループページ）の**日程表で対象の日を選び**、一括操作バーのボタンを押す（2026-08-20 変更。それ以前は `/events/[id]` の進行管理トグル＋伝播確認ダイアログだった）。前進3種はボタン、後退3種は「戻す操作」の `<details>` の中。既定は選択可能な日すべてにチェックで、`cancelled` の日は選択できない。前進2ボタンは選択した日のうち1日でも状態が動くときだけ**描画**され、後退3ボタンは同じ条件で**活性**になる（`applied` から直接 `not_applying` へは遷移させず「未申込に戻す」を経由する2手順は維持。状態機械を単純に保つため）。「支払済にする」の表示条件だけは申込済の日を要求する点でサーバー側のガード（`payment_type='advance'` かつ `payment_status='unpaid'` のみ。`entry_status` は見ない）より厳しい —— 申込済と未申込が混ざった選択ではボタンが出て両方が支払済になる
