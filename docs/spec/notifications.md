...
- **送信**: 人数は push の前に保存し、**push が成功したときだけ** `last_sent_at` / `last_sent_by` を進め、同時に `last_error` を NULL へ戻す（「送信済」と「送信に失敗しました」が同時に出ないようにする）。失敗時は `last_attempted_at` / `last_error` だけを書く（送信済みにせず再送できる状態のまま残す）。人数が全級0のときは送信させない（記録も作らない — `total_jpy` は NOT NULL なので 0 円の送信記録を生やさない）。再送は要綱再送・オープンチャット再送と同じ扱いで何度でもできる。
