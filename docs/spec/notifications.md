...
- LINE APIが401を返した場合はチャネルを `disabled` にして紐付けを `revoked` にする（トークン失効）。403 / 404 はチャネルを `available` に戻して紐付けのみ `revoked` にする（Bot追放・宛先消失）。★**400（メッセージ内容の不備）では紐付けを解除しない** — `textV2` の導入でペイロード起因の400が起こりうるようになっており、宛先と無関係な不備で正常な紐付けを壊すと、その大会の通知が以後すべて止まる。429（レート制限）もリトライ可能として残す。いずれも「送信開始時に保持していたチャネル/グループ」が現在も有効な場合に限り実行し、レース中の再紐付けを壊さない。
...
会計が参加費を振り込んだあと、**振込明細の写真を証憑として北溟に登録し、同じ操作で大会 LINE グループへ「振込完了＋北溟上の想定金額＋明細の写真」を流す**（`/admin/entries/[groupId]` の一括操作「支払報告」。2026-09-04 新設。[features/payment-receipt-broadcast/requirements.md](../features/payment-receipt-broadcast/requirements.md)）。
...
確定名簿が出たグループの会計へ、振込金額を LINE で連絡する。**手動送信のみ・全自動送信はしない**。

導線は**2つ**あり、どちらも同じ文面・同じ送信記録（`entry_group_payment_notices`。1グループ1行）・同じ送信処理（`lib/events/payment-notice-send.ts` の `sendPaymentNoticeCore`）を共有する。どちらから送っても両画面の「送信済」表示が同じ値を指す。

1. **申込グループページ**（`/admin/entries/[groupId]` の「振込連絡」セクション）— 名簿確定後にいつでも送れる恒久導線
2. **メール処理画面**（`/admin/mail-inbox/mail/[id]` の統合処理フォーム内「会計へ振込連絡を送る」）— 確定名簿メールを処理する流れの中でそのまま送る導線
...
- **送信**: 人数は push の前に保存し、**push が成功したときだけ** `last_sent_at` / `last_sent_by` を進め、同時に `last_error` を NULL へ戻す。★**表示は `last_error` を優先する** — 「失敗 → 成功」では失敗表示が消えるが、「成功 → 再送失敗」では `last_sent_at` が過去の成功のまま残るため、失敗中は見出しも「送信失敗」にし、過去の成功は「最終成功」と明示して別に出す（相反するラベルを同時に出さない）。失敗時は `last_attempted_at` / `last_error` だけを書く（送信済みにせず再送できる状態のまま残す）。人数が全級0のときは送信させない（記録も作らない — `total_jpy` は NOT NULL なので 0 円の送信記録を生やさない）。再送は要綱再送・オープンチャット再送と同じ扱いで何度でもできる。
- **メール処理画面からの送信**（統合処理フォームの「会計へ振込連絡を送る」）:
  - **露出条件は種別＝確定名簿 ∧ 対象グループ選択済みの2つだけ**。★`settled` を条件に入れない — この画面での処理そのものが `mail_kind='confirmed_roster'` ∧ `triage_status='processed'` を書いて確定名簿シグナルを成立させるので、処理**前**に見ると必ず false になる。添付を1件も採用しない確定連絡メール（本文だけで確定を伝える型）でも同じく出る
  - 対象日が無いときも**セクション自体は出し、送信だけを不可にして理由を1行で示す**（黙って消さない）。判定の正典は `lib/events/payment-notice-availability.ts`（DB 非依存の純関数。client からも import される）で、優先順位は 未申込 → 現地払い → 支払済 → LINE未紐付け → 単価を解決できる級なし
  - **支払締切・振込先をその場で入力できる**（確定名簿メールの処理時点では未設定のことがある）。入力値は `propagateFieldsToGroup` で**グループ内の全イベント（中止の日を含む）**へ保存する（既存の共通項目一括保存と対象を揃える）。★**保存は送信可否と切り離す** — セクションが描かれている限り、送信チェックの ON/OFF によらず保存する（ゲートが掛かるのは push だけ）。セクション自体が出ていないときは保存もしない
  - **振込先は送信するときだけ必須**。支払締切は任意（空なら1通目の日付行が消えるだけ）
  - ★**送信できない状態（未申込・現地払い・支払済・LINE未紐付け・単価を解決できる級なし）でも、振込期限・振込先の入力欄は出して保存できる。** ゲートが掛かるのは push だけで、保存は種別＝確定名簿 ∧ グループ選択済みの2条件だけを見る。ここで保存まで止めると、確定名簿メールを処理する時点に振込先を入れる場所が無くなり、この機能が無くそうとしている「グループページへ移動して入れ直す」状態に戻る
  - 送信時（`after()` の push 直前）の再検証で対象外になった場合は、**黙って落とさず試行記録に残す**（`last_attempted_at` / `last_error`）。`processMail` は既に `{ok:true}` を返してメール一覧へ戻っているため、記録しないと管理者はどの画面でも脱落に気づけない。メール取り消し（世代トークン不一致）は正常な中止なので記録しない
...
- `message`（招待コード**以外**のテキスト）: チャットコマンド（下述 line-chat-commands）。
- その他（`follow`、`memberJoined` 等）は no-op。

### line-chat-commands: グループ発言による申込/支払の進行

大会別グループで Bot をメンションして「申し込みました」「振り込みました」と送ると、北溟を開かずに進行管理のステータスが進む。北溟側の画面は増えない（入口は LINE の発言だけ、出口は既存の進行管理表示と既存のライフサイクル通知）。

発火は次を**すべて**満たしたときだけ:

1. `purpose='event_broadcast'` のチャネル（級別グループでは対象大会を特定できないため構造的に発火しない）
2. Bot 自身がメンションされている（`mentionees[].isSelf`、または `mentionees[].userId` が `line_channels.webhookDestinationId` と一致）
3. 本文に対象語を含む（申込: 申込 / 申し込み / 申し込ん / 申込完了 / 申込済 / 申し込みました / 申し込んだ、支払: 振込 / 振り込み / 振り込ん / 振込完了 / 振り込みました / 入金しました / 支払いました / 払いました）。**語の判定はメンション文字列を除去してから**行う（Bot 表示名が語を含むと、メンションしただけで発火してしまうため）
4. その チャネルの `event_line_broadcasts` が `status='linked'` かつ `line_group_id` が発言元グループと一致する
5. 発言者に権限がある（`source.userId → users.line_user_id → role` を毎回 DB から引く。申込＝admin のみ／支払＝admin・vice_admin。解決できなければ実行しない fail-closed。`is_treasurer` は認可に使わない）
6. 本文に否定表現（ません / ない / まだ / 未）を含まない

対象は申込グループ内の `cancelled` でない全開催日（LINE 起点では日を選ばせない）。申込は既存ガード `WHERE entry_status='not_applied'` の母集団だけが動くので、「今回は申し込まない」（`not_applying`）と決めた日は自然に除外される。支払は `payment_type='advance'` の日だけ。状態遷移・once-ever の claim・通知は進行管理画面と**同じ共通ロジック**（`lib/events/apply-entries-applied.ts` / `lib/events/apply-payments-paid.ts`）を通る。

**成功時は返信しない。** 既存のライフサイクル通知（申込＝2通／支払＝1通）が同じグループへ流れ、それが完了報告になる。返信するのは「すでに完了している」「否定表現で判定できない」「現地払いが混ざって一部だけ進めた」「進められる日が無い」「実行に失敗した」の5系統だけで、**権限が無い / メンションが無い / 語を含まない / 紐付いていないグループ**の発言には一切返信しない（グループを荒らさないため）。申込と支払の両方が返信対象になった場合も replyToken は単発なので、1回の reply に複数メッセージを載せる。

取り消し・巻き戻しの経路は作らない（画面から戻す）。誰が LINE から実行したかは専用テーブルには残さず、構造化ログにのみ出る（画面操作も実行者を記録していないため揃えている）。
...
**配信**は 1. と同じ大会別 Bot グループへ **Flex 1通のみ**（級別に LINE グループを分けない）。**抽出シートは保存だけを行い、その場では送らない** — 送るのはメール本文・添付と同じ「実行する」で、統合処理フォームの「LINE 配信」内の子チェック「オープンチャットの招待リンクも送る（N件）」を入れたときだけ（`processMail` が `after()` の中で本文配信の直後に push する）。抽出直後と実行時の2回に分かれて届いていたのを1回にまとめたもの。バブル内のテキストは「大会オープンチャット」と大会名の2つだけで説明文・注意書きを置かず、本文は各行のボタンを縦一列に並べる（carousel はスワイプしない人が他の級に気づかない）。ボタンは `uri` アクションで URL をトークに露出させず、パスワードのある行はボタン直下に表示する。**再配信は毎回全件を送る**（差分配信は「前に何が送られたか」を受け手が覚えている前提になる）ので、**2回目以降は既定でチェックを外す**（配信済み回数を脇に出す。ただし前回配信より後に増えた行があるときは既定 ON に戻す）。チェック＋「実行する」の2アクションが誤送信のゲートで、統合処理フォームでは確認ダイアログを出さない。全件のラベルを列挙し前回配信以降に増えた行に「（今回追加）」を付けた確認ダイアログは、申込グループページの再送ボタン（下記）に残っている。

★**配信の記録は `entry_group_open_chat_broadcasts`（追記専用ログ）に持ち、`event_broadcast_messages` には一切書かない。** 同テーブルの `UNIQUE(event_line_broadcast_id, mail_message_id)` は「1メール=1配信」を DB レベルで強制するため「毎回全件」と原理的に両立せず、同じメールからの2回目の配信が制約違反で落ちる。この UNIQUE を緩める解決は禁止 — メール配信の冪等性は本機能と無関係に守られなければならない。binding の再検証（push 直前に紐付けが変わっていないか）と push 失敗時の復旧（401 → channel を disabled + binding revoke／403・404 → binding のみ revoke／400 は何もしない）は `line-broadcast.ts` から切り出した共通ヘルパーを使い、**ヘルパー自身は監査行を書かない**（記録は呼び出し側の責務）。
...
- **`/(app)/admin/mail-inbox/mail/[id]`**: 統合処理フォーム内の「会計へ振込連絡を送る」セクション（種別＝確定名簿 ∧ 対象グループ選択済みのときだけ描画）。級ごとの人数入力（**単価は表示のみ**）・支払締切・振込先・文面プレビュー・送信チェックを持つ。送信できないときはセクションを出したまま理由を1行で示す。画面全体の構成は `spec/mail-worker.md`。
...
3. 大会グループで Bot をメンションして「申し込みました」「振り込みました」と送っても 1. / 2. と同じ遷移・同じ通知が起きる（上記 line-chat-commands）。認可は webhook 側で解決し、状態遷移は同じ共通ロジックを通る。
...
確定名簿メールを処理する流れの中で送る場合は 2.〜3. の代わりに:

2'. 管理者がメール詳細の統合処理フォームで種別＝確定名簿・対象グループを選ぶ → 「会計へ振込連絡を送る」セクションが出る（未送信なら既定 ON）。人数・支払締切・振込先を確認して「実行する」。
3'. 採用・triage のコミット後に、LINE 配信 →（オープンチャット）→ 振込連絡の順で push する。支払締切・振込先はグループ内の全日へ保存され、グループページの表示にも反映される。
