...
6. 成功すれば `classify/evidence.ts` で `regional_eligibility[].evidence_quote` を照合コーパス（手順 4 で LLM 入力と同時に集めた本文＋送った添付の抽出テキスト）と機械照合し、各要素へ `evidence_verified` を書き足してから常に `tournament` を返す。**AI は「大会案内かどうか」を判定しない**（管理者が押した時点でその判断は済んでいる）
...
**PROMPT_VERSION 3.1.0** で `EventUnit` に `regional_eligibility[]`（D・E 級の地域制限判定）が加わった。その単位の `eligible_grades` に含まれる **D 級・E 級だけ**を対象に 1 級 1 要素で、`verdict`（「制限なし」「北海道は対象」「北海道は対象外」「要確認」の 4 値）と `evidence_quote`（判定の根拠にした箇所を資料から**一字一句そのまま**抜き出した一文。「要確認」だけ null 可）を持つ。D・E を含まない単位は空配列。ホーム地域「北海道」と対象級「D・E」は `classify/regional.ts` の定数 1 箇所に置き、プロンプト・スキーマ・承認フォームの文言がすべてそこから合成される（他会へ配布するときの差し替え箇所を 1 つにするため）。「北海道の選手」とは北海道に在住・在勤・在学する、または北海道の会に所属する選手を指す（会員個人の事情は見ない）。抽選の優先枠（地元枠）は地域制限ではなく「制限なし」、所属会を条件にする書き方は「北海道は対象外」、D・E で条件が異なる案内は級ごとに別々に判定、資格の記載が見当たらなければ「制限なし」と推測せず「要確認」、という判定規則はシステムプロンプトが指示する。`superRefine` は同一単位内の級の重複、「要確認」以外で根拠が null／空白のペイロード、そして**判定の級集合が `eligible_grades` の D・E と一致しない**（含まれる級の判定が欠落している、または含まれない級の判定がある）ペイロードを拒否する —— 欠落を通すと承認フォームに警告が出ないまま対象級が残り、AI の見落としがそのまま人の見落としになるため、他の契約違反と同じくリトライ→`ai_failed` の経路へ倒す。

**根拠の一文の機械照合**（`classify/evidence.ts`）: `classifyMail` は AI に渡した入力（本文＋選択して実際に送った添付の抽出テキスト。PDF は `extractedText`、text 添付と lazy fallback はそのテキスト。未選択・`failed` の添付は含めない）を照合コーパスとして同時に集め、`llm.extract` 成功後に各要素へ `evidence_verified`（双方を空白類除去＋Unicode NFKC で正規化したうえで、根拠がコーパスの部分文字列なら true）を書き足してからドラフトへ保存する。正規化はこの 2 つだけで、句読点無視・編集距離などの寛容化はしない（空白を無視するのは PDF の抽出テキストに「202 6 年」のような抽出由来の空白が混じるため）。抽出テキストを持たない画像だけの PDF に根拠がある場合は必然的に未照合になる —— 異常ではなく、自動で外さず人が見る側へ倒すための設計。照合結果は判定を書き換えない（自動で外すかどうかだけが照合結果に従う）。`evidence_verified` はワーカー専用項目で、`llm/anthropic.ts` が Anthropic へ渡す tool の input_schema からこの項目だけを削る（パスが見つからなければ例外にして黙って無効化させない。AI が出力しても照合で必ず上書きされる）。判定・根拠・照合結果はドラフトの抽出 payload 内にだけ持ち、`events` へは持ち回さない（対象級から外れれば申込・級別配信・未回答アラートは既存の `eligible_grades` の仕組みで止まる）。
...
「種別 → 対象の大会 → 実行」の 1 フォームに畳んだ処理導線。上から **種別**（未選択 / 大会案内 / 申込名簿 / 確定名簿 のセグメント）→ **対象の大会**（申込グループ単位。`GroupPickerSheet`）→ **採用する名簿ファイル**（名簿種別のみ）→ **発表日**（名簿種別のみ）→ **オープンチャット**（抽出シート `OpenChatExtractSheet` の起点。保存のみで配信しない。[spec/notifications.md](notifications.md)）→ **LINE 配信** → **会計へ振込連絡**（種別 = 確定名簿 のときだけ。[spec/notifications.md](notifications.md)）→ **実行する**。選んだ種別に応じて必要な欄だけが DOM に生える。
...
「会計へ振込連絡」は種別 = 確定名簿 ∧ グループ選択済みのときだけ現れ、級ごとの人数（単価は表示のみ）・振込期限・振込先・文面プレビュー・送信チェックを持つ。★**`settled`（確定名簿あり）を露出条件に入れない** — この画面での処理そのものが確定名簿シグナルを成立させるので、処理前に見ると必ず false になる。送信できない状態（未申込・現地払い・支払済・LINE未紐付け・単価を解決できる級なし）でもセクションは残し、理由を1行で出して送信だけを止める。振込期限・振込先はグループ内の全日（中止の日を含む）へ保存され、**保存は送信可否と切り離す**（送信チェックを外しても、そもそも送れない状態でも保存はする。ゲートが掛かるのは push だけ）。仕様の正典は [spec/notifications.md](notifications.md)「payment-notice」。
...
**D・E 級の地域制限**（`admin/mail-inbox/regional-eligibility-utils.ts` の `planRegionalEligibility` と `components/RegionalEligibilityNotice.tsx`）: 各単位の `regional_eligibility` を読み、**判定が「北海道は対象外」かつ根拠が照合済み（`evidence_verified: true`）かつその級が `eligible_grades` に含まれる**ときだけ、その級を対象級の初期値から外す（`composeTitle` の入力にも反映され、兵庫 A〜E で D・E が外れれば「兵庫ABC」）。外した級ごとに級名と根拠の一文を含む警告を、単位カードの「このイベントを登録する」と入力欄の間（`fieldset` の外＝未チェックでも読める位置）に出す。未照合の「北海道は対象外」と「要確認」は外さず、根拠の一文（無ければ「資格の記載が見当たりません」）付きで確認を促す。「北海道は対象」は外さず根拠付きの補足を出し、「制限なし」はフォームに出さない。単位の全ての級が外れたときは「このイベントを登録する」を既定 OFF にして却下を促す（ON に戻せば通常どおり登録できる）。地域制限で級を1つでも外した単位は、級を1つも選ばずに送信しようとするとクライアント側で止めて案内する —— `eligible_grades` が null で保存されると既存仕様では「全級対象」の意味になり、外したはずの級まで対象に戻ってしまうため（全級が外れた単位で登録だけ ON に戻した場合も、一部の級が外れた単位で残りを手動で外した場合も同じ）。AI が級を読めなかった単位（元から null）は従来どおり通す。外すのは**フォームの初期値だけ**で、`approveDraftUnits` は送信された対象級をそのまま保存し判定を再適用しない（人間の判断が AI より上位。`source_mismatch` と同じ原則）。登録済み（materialize 済み）の単位と、`regional_eligibility` を持たない 3.0.x 以前のドラフトでは何も外さず警告も出ない。承認はブロックせず、新しいステータスも作らない。警告ブロックは `source_mismatch` バナーと同じ警告トークンで描く。

`ExtractedPayloadView`（AI 抽出結果の折りたたみ）には単位ごとに「地域制限（D・E 級）」の小表（級／判定／根拠の一文／照合: 照合済み・未照合・—）が出る。フォームに出ない「制限なし」もここでは全件見られる。
