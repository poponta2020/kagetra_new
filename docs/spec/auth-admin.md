...
**role-preview-switch（表示ロールの一時プレビュー）**: `session.user.role` は上記の認可判定がそのまま参照する**実効ロール**であり、`session.user.realRole` が DB 由来の**本物のロール**を保持する。両者は通常時（プレビュー未使用）は同値。環境変数 `ROLE_PREVIEW_USER_IDS`（カンマ区切りの `users.id`）で許可されたユーザーだけが、実効ロールを本物のロール以下へ一時的に落とせる（JWT クレーム `viewAsRole` として保持。DB の `users.role` は変更しない）。実効ロールの生成点は `apps/web/src/auth.config.ts` の `session` コールバック 1 箇所のみで、ここが UI の出し分けと Server Action / route handler の認可判定の両方に同時に反映される。プレビューの開始・終了を認可する条件（許可リスト所属・切替先が `selectableRoles(realRole)` に含まれること）は**必ず `realRole` で判定し、実効ロール（`role`）を使わない**。実効ロールで判定すると、プレビュー中に自分自身を締め出して管理者へ戻れなくなるため。切替 Server Action は `apps/web/src/app/(app)/role-preview-actions.ts`。

切替先は原則「本物のロール以下」だが、**`guest` だけは本物のロールが `admin` のときにしか選べない**（許可リストに載った副管理者・一般会員にゲストビューが生えないようにするため）。この admin 限定は `selectableRoles()` が内包し、UI の選択肢生成・Server Action・jwt コールバックの 3 経路が同じ関数を通る。加えて実効ロールの生成点 `resolveEffectiveRole()` にも `view === 'guest' && real !== 'admin'` の防御を置く — ランク比較だけだと `guest`(最下位) が全ロールの下に収まってしまい、改竄 JWT で副管理者・一般会員をゲストビューへ落とせるため（この 1 行は冗長ではない）。ゲストビュー中の設定ページには「表示ロール」セクションだけが描画され、それが唯一の復帰導線になる（[spec/ui-shell.md](ui-shell.md)）。**本物のゲスト**（`realRole === 'guest'`）はプレビュー機能を一切持たない。
