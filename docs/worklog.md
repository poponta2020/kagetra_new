...
- **残 DoD:** 本番は **PR ship 済だが migration 0037 適用 + backfill は投入待ち**（[[project_bulk_load_handover]] と同じく series 層含む dump/apply・db:migrate 運用・db:push 禁止）。本番実機目視。**次＝PR-2 ナビ（#212, タスク4・PR-1 と独立）**。
