...
- 会員100名超、LINE 通知対象約50名/年。招待制・LINE 認証のみ
- 旧システムと並行稼働中（P4 完了まで）。本番: new.hokudaicarta.com（Oracle Cloud 東京）
- API 実処理は apps/web（Server Actions + route handlers）にある。apps/api（Hono）はスケルトン。技術スタック・構成はリポジトリルートの `CLAUDE.md` を参照
...
| ドメイン | 責務 | ファイル |
|---|---|---|
| イベント・出欠 | 大会申込イベントの一覧・作成/編集・出欠回答・進行管理（申込/支払い）・アーカイブ | [spec/events-attendance.md](spec/events-attendance.md) |
