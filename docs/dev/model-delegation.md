...
> **【2026-07-10 更新】devflow プラグイン移行に伴い、モデル運用の正典は移動した:** セッション既定は `opusplan`（Plan=Opus/実行=Sonnet）+ Advisor=Opus、設計セッションは `/model opus`、サブエージェント委譲基準は「モデルコスト」から「**コンテキスト隔離・作業独立性**」に変更（正典 = devflow プラグインの implement スキルと `.claude/project-profile.md`）。以下の記述は移行前の設計（4条件ゲート・main固定・Haiku明示）の記録として残す。運用ルール節の実務知見（小径修正は委譲しない・worktree分離・テスト要件をbriefに含める等）は引き続き有効。
...
- **品質フィードバックループ**: 受け入れ確認 NG が同種タスクで2回続いたら、その種別を委譲対象から外して本ドキュメントを更新する。.claude/memory/ の実装記録には「どのワーカー（モデル）が実装したか」を含め、後から監査できるようにする（静かな能力ミスマッチ対策）
